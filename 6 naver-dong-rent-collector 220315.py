...
for i in range(10):
    ...
    if 'articleList' in data:
        ...
        'cache-control': "no-cache"
            ...
            print(j)
            j += 1
            list_data.append([articleNo, warrantPrice, rentPrice, priceBySpace, articleName, area1, area2, bcRat, vlRat, usageAreaTypeName, lawUsage, floorInfo, directionTypeName, etcParkInfo, \
                              buildingUseAprvYmd, newPlatPlc, articleConfirmYMD, realtorName, representativeName, cellPhoneNo, cpPcArticleUrl])
columns = ['매물번호', '보증금', '월세', '평단가', '건물명', '대지면적', '연면적', '건폐율', '용적률', '토지용도', '건물용도', '층수', '방향', '주차', '사용승인일자', '물건주소', '확인일자', '중개사무소', '중개인', '전화', '블로그']
real_df = pd.DataFrame(list_data, columns=columns).sort_values(by='대지면적', ascending=False)
real_df.head()
real_df.info()
real_df.to_excel('./6 ' + dong + '지역 네이버부동산 ' + type + ' 월세매물조회 ' + curr_date + '.xlsx', sheet_name=f"{dong}", index=False)
