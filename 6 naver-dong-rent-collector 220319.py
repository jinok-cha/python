...
for dongs in dong_list:
    if dong == dongs[1]:
        ...
        break
for i in range(10):
    url = 'https://new.land.naver.com/api/articles?cortarNo=' + dong_code + '&order=rank&tradeType=B2&realEstateType=' + type + '&priceType=RETAIL&page=' + str(i+1)
    payload={}
    headers = {
    ...
    if 'articleList' in data:
        for list in data['articleList']:
            ...
            payload = {}
            headers = {    'Authorization': 'Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpZCI6IlJFQUxFU1RBVEUiLCJpYXQiOjE2NDczMjA4NDAsImV4cCI6MTY0NzMzMTY0MH0.CVMz-aQokatAj0AXsYi6maU5zvzYTBurOQfhW7XUpnU',
        'User-Agent': "PostmanRuntime/7.20.0",
        'Accept': "*/*",
        'Cache-Control': "no-cache",
        'Postman-Token': "adbba748-cb85-4fb4-8f6a-4be441f19cc3",
        ...
        'cache-control': "no-cache"
            ...
            try:
                ...
                pass
            try:
                currentUsage = data1['articleBuildingRegister']['currentUsage']
            except:
                currentUsage = ''
                pass
            try:
            ...
            print(j)
            j += 1
            list_data.append([articleNo, floorInfo, warrantPrice, rentPrice, priceBySpace, articleName, exclusiveSpace, supplySpace, currentUsage, lawUsage, directionTypeName, etcParkInfo, \
                              buildingUseAprvYmd, newPlatPlc, articleConfirmYMD, realtorName, representativeName, cellPhoneNo, cpPcArticleUrl])
    time.sleep(5)
columns = ['매물번호', '층수', '보증금', '월세', '평단가', '건물명', '전용면적', '계약면적', '현재업종', '건물용도', '방향', '주차', '사용승인일자', '물건주소', '확인일자', '중개사무소', '중개인', '전화', '블로그']
real_df = pd.DataFrame(list_data, columns=columns).sort_values(by='전용면적', ascending=False)
real_df.head()
real_df.info()
real_df.to_excel('./6 ' + dong + '지역 네이버부동산 ' + type + ' 월세 매물조회 ' + curr_date + '.xlsx', sheet_name=f"{dong}", index=False)
