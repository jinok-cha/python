...
search_area = input('검색할 도시지역을 입력하세요 : ')
...
for gungus in gungu_list:
    ...
    for dongs in dong_list:
        dong_code = dongs[0]
        print(search_area, gungus[1], dongs[1], dong_code)
        for i in range(5):
            url = 'https://new.land.naver.com/api/articles?cortarNo=' + dong_code + '&order=rank&realEstateType=GM&priceType=RETAIL&page=' + str(i+1)
            payload={}
            headers = {
            ...
            if 'articleList' in data:
                ...
                'cache-control': "no-cache"
                    ...
                    print(j)
                    j += 1
                    list_data.append([gungus[1] + ':' + dongs[1], articleNo, dealPrice, priceBySpace, noneAptBuildingName, area1, area2, bcRat, vlRat, usageAreaTypeName, lawUsage, floorInfo, directionTypeName, etcParkInfo, \
    ...
    time.sleep(60)
columns = ['지역', '매물번호', '매매가격', '평단가', '건물명', '대지면적', '연면적', '건폐율', '용적률', '토지용도', '건물용도', '층수', '방향', '주차', '사용승인일자', '보증금', '월세', '부동산가치', '확인유무', '물건주소', '확인일자', '중개사무소', '중개인', '전화', '블로그']
real_df = pd.DataFrame(list_data, columns=columns).sort_values(by='대지면적', ascending=False)
real_df.head()
real_df.info()
real_df.to_excel('./6 ' + search_area + '지역 네이버부동산 매물조회 ' + curr_date + '.xlsx', index=False)