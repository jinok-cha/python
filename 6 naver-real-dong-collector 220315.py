...
columns = ['매물번호', '매매가격', '평단가', '건물명', '대지면적', '연면적', '건폐율', '용적률', '토지용도', '건물용도', '층수', '방향', '주차', '사용승인일자', '보증금', '월세', '부동산가치', '확인유무', '물건주소', '확인일자', '중개사무소', '중개인', '전화', '블로그']
real_df = pd.DataFrame(list_data, columns=columns).sort_values(by='대지면적', ascending=False)
real_df.head()
real_df.info()
real_df.to_excel('./6 ' + dong + '지역 네이버부동산 ' + type + ' 매물조회 ' + curr_date + '.xlsx', sheet_name=f"{dong}", index=False)
