...
url = "https://m.land.naver.com/map/35.1576627:129.0590554:18:/SG:SMS:GM/A1:B1:B2#mapList" #input("검색할 지역의 url을 입력하세요 :")
...
cortarNo = "2623010300" #input("cortarNo를 입력하세요  : ")
...
for v in values[:10]:
    ...
    for idx in range(1, math.ceil(len_pages)):
        ...
        try:
            print(hanPrc, rentPrc, float(hanPrc.replace(",","").replace("억","0000").replace("억 ", "")))
            avg_hanPrc = float(hanPrc.replace(",","").replace("억","0000").replace("억 ", "")) / float(spc2) * 0.3025
            avg_rentPrc = 0#float(rentPrc.replace(",", "")) / float(spc2) * 0.3025
            print(avg_hanPrc, rentPrc)
        except:
            avg_hanPrc = 0
            avg_rentPrc = 0


        list_data.append([atclNo, rletTpNm, tradTpNm, prc, spc1,
           spc2, hanPrc, rentPrc, avg_hanPrc, avg_rentPrc, flrInfo,
           lat1, lng2, tagList, rltrNm, detaild_information])
        time.sleep(3)
columns = ['물건번호', '상가구분', '거래방식', '매매가', '계약면적', '전용면적', '보증금', '월세', '평당보증금', '평당월세', '층수', '위도', '경도', '기타정보', '부동산', '비고']
