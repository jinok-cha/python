...
cortarNo = input("cortarNo를 입력하세요  : ")
...
for v in values[:20]:
    ...
    for idx in range(1, math.ceil(len_pages)):
        ...
        try:
            ...
            spc1 = round(float(json_str1['body'][0]['spc1']) * 0.3025, 2)   # 계약면적(m2) -> 평으로 계산 : * 0.3025
            spc2 = round(float(json_str1['body'][0]['spc2']) * 0.3025, 2)   # 전용면적(m2) -> 평으로 계산 : * 0.3025
...
real_df = pd.DataFrame(list_data, columns=columns).sort_values(by='전용면적(평)', ascending=False)
