    ...
    for idx in range(1, math.ceil(len_pages)):
        ...
        for i in range(len(json_str1['body'])):
            ...
            list_data.append([atclNo, rletTpNm, tradTpNm, prc, spc1, spc2, int(hanPrc), rentPrc, avg_hanPrc, avg_rentPrc, flrInfo[0], flrInfo[1],
               lat2, lng2, tagList, rltrNm, detaild_information])
            #time.sleep(2)
        #time.sleep(5)
...
if not os.path.exists('./네이버부동산 매물수집_' + curr_date + '.xlsx'):
    with pd.ExcelWriter('./네이버부동산 매물수집_' +curr_date + '.xlsx', mode='w', engine='openpyxl') as writer:
        real_df.to_excel(writer, sheet_name=f"{keyword}")
else:
    with pd.ExcelWriter('./네이버부동산 매물수집_' + curr_date + '.xlsx', mode='a', engine='openpyxl') as writer:
        real_df.to_excel(writer, sheet_name=f"{keyword}")